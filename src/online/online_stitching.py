...
@capture_timing_info()
def generate_keypoints(matcher, image_a: numpy.ndarray, image_b: numpy.ndarray, device: str, index: int) -> \
        tuple[numpy.ndarray, numpy.ndarray]:

    tensor_a = frame_to_tensor(image_a, device)
    tensor_b = frame_to_tensor(image_b, device)

    prediction: MatchingOutput = matcher(MatchingInput(image0=tensor_a, image1=tensor_b, index=index))
