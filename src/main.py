...
from util.metrics import capture_timing_info, print_metrics
...
def generate_keypoints(matcher, image_a: numpy.ndarray, image_b: numpy.ndarray, device: str) -> tuple[
    ...
    tensor_a = frame2tensor(image_a, device)
    tensor_b = frame2tensor(image_b, device)
    ...
    matches, confidence = prediction["matches0"], prediction["matching_scores0"]
...
@capture_timing_info()
def do_stitching(config: Config):
    ...
    image_paths.sort(key=lambda x: int(basename(x)[4:-4]))
    ...
    matcher_config = {
        ...
        "superglue": {
            "weights": config.superglue_weights,
            "sinkhorn_iterations": config.sinkhorn_iterations,
            "match_threshold": config.match_threshold,
        }
    }
    matcher = Matching(matcher_config).eval().to(config.device)
    ...
    for i, (image_a, image_b) in enumerate(image_pairs):
        ...
        if not mkpts0.shape[0] > 0 or not mkpts1.shape[0] > 0:
            print(f"[INFO]: No Keypoints extracted in iteration {i} when matching {basename(image_path_pairs[i][0])} with {basename(image_path_pairs[i][1])}!")
        ...
        if matrix is None:
            print(f"[INFO]: Unable to estimate transform between {basename(image_path_pairs[i][0])} and {basename(image_path_pairs[i][1])} on iteration {i} ! ")
            print(f"[INFO]: Continuing!")
            previous_image = image_b
            next_image_offset = Point()
            current_size = Point(previous_image.shape[1], previous_image.shape[0])
